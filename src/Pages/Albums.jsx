...
export default function Albums() {
  ...
  const {currentAlbums, getUserName, totalAlbums, handlePageChange, currentPage, isDark} = useContext(DashboardContext)
  
  return (
    <>
        <div className={`container mx-auto p-4 ${ isDark ? "bg-gray-300" : "bg-gray-100"} rounded-lg shadow-lg max-w-3xl w-full h-full`}>
            ...
            {currentAlbums.map((item, index) => (
              <Card shadow="sm" key={index} className={`${isDark ? "bg-[#18181B] text-[#686871]" : "bg-white"}p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200`}>
